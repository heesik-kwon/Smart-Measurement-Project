...
| ③    | 19,040           | `3 → 4` | SYNC_L에서 dht11_io == 1 & w_tick 발생 → SYNC_H(4)로 전이 |


### ⏱️ Tick Timing 분석

- `tick_cnt_reg`가 **1900**일 때 FSM 상태는 START(1) → 이후 1 클럭 후 0으로 초기화됨
- 이는 10us * 1900 = **19ms**로, 데이터시트에서 요구하는 **MCU START 신호 길이**와 일치
- 이후 `tick_cnt_reg`는 다시 증가하며 `w_tick` 발생 시점을 기준으로 FSM이 다음 상태로 전이됨
...
| 약 40비트 동안 반복 | 총 40회 반복 | DHT11에서 습도, 온도, 체크섬 총 40비트 수신 |
